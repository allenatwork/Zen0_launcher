...
public class Widgets {
    ...
    public Widgets(MainActivity mainActivity, ViewGroup vgWidgerArea) {
        this.mainActivity = mainActivity;
        this.prefs = mainActivity.prefs;
        widgetArea = vgWidgerArea;
    }
    ...
    private void restoreWidgets() {// remove empty list view when using widgets, this would block touches on the widget
    ...
    public static final long ANIM_TIME = 200L;

    public void show(boolean show) {
        if (show) {
            if (widgetArea.getVisibility() != View.VISIBLE) {
                Tool.visibleViews(ANIM_TIME, widgetArea);
            }
        } else {
            if (widgetArea.getVisibility() == View.VISIBLE) {
                Tool.invisibleViews(ANIM_TIME, widgetArea);
            }
        }
    }
}
